...
namespace HexasphereGrid_Demos {
	public class Demo1 : MonoBehaviour {
		...
		[Tooltip("Sample material used to render a transparent arrow on top of a tile.")]
		public GameObject mathmagicCircle;
		
		[Tooltip("Sample material used to render a transparent arrow on top of a tile.")]
		public GameObject hexagonItemGameObject;
		
		[Tooltip("Sample material used to render a transparent arrow on top of a tile.")]
		public GameObject hexagonExperienceGameObject;
		
		[Tooltip("Sample material used to render a transparent arrow on top of a tile.")]
		public GameObject metaverseItem;
		...
		const int ARROW_TILE_INDEX = 0;
		const int MATHMAGIC_INDEX = 12;
		
		const int HEXAGON_ITEM = 33;
		const int HEXAGON_EXPERIENCE = 34;
		const int HEXAGON_03 = 39;
		const int HEXAGON_04 = 40;
		
		const int METAVERSEITEM_INDEX = 7;
		void Start () {
			...
			// HexagonToken01(HEXAGON_ITEM);
			// HexagonToken02(HEXAGON_EXPERIENCE);
			//HexagonToken03(HEXAGON_01);
			//HexagonToken04(HEXAGON_01);

			//StartMetaverseItem(METAVERSEITEM_INDEX);
		}
		...
		void OnGUI () {
			...
			if (GUI.Button (new Rect (10, 185, 220, 30), "Assemble Mode")) {
				if (selectMode != DEMO_MODE.Assemble) {
					selectMode = DEMO_MODE.Assemble;
				} else {
					selectMode = DEMO_MODE.Idle;
				}
			}
			...
			paintInColor = GUI.Toggle (new Rect (10, 220, 200, 20), paintInColor, "Color");
			paintInColor = !GUI.Toggle (new Rect (10, 240, 200, 20), !paintInColor, "Texture");
			if (prevPaintColor != paintInColor) {
				selectMode = DEMO_MODE.Paint;
			}

			if (GUI.Button (new Rect (10, 260, 100, 30), "Colorize")) {
				RandomColors ();
			}

			if (GUI.Button (new Rect (120, 260, 110, 30), "Extrusion")) {
				if (hexa.extruded) {
					hexa.extruded = false;
					RandomColors ();
			...
			if (GUI.Button (new Rect (10, 300, 220, 30), "Random Obstacles")) {
				RandomObstacles ();
			}

			if (GUI.Button (new Rect (10, 340, 220, 30), "Clear Tiles")) {
				ClearTiles ();
			}

			if (GUI.Button (new Rect (10, 380, 100, 30), "Random Path")) {
				RandomPath ();
			}

			if (GUI.Button (new Rect (120, 380, 110, 30), "Custom Path")) {
				selectMode = DEMO_MODE.PathFind_StartTile;
			}

			if (GUI.Button (new Rect (10, 420, 220, 30), "Load Heightmap")) {
				if (divisions < 50) {
		...
		void StartMetaverseItem(int tileIndex)
		{
			GameObject obj = Instantiate(metaverseItem);
		...
		public void InstanciateNewGameObjectPosition(GameObject newGameObject, int indexTile)
		{
			GameObject obj = Instantiate(newGameObject);

			// Move object to center of tile (GetTileCenter also takes into account extrusion)
			//int tileIndex = Random.Range (0, hexa.tiles.Length);
			obj.transform.position = hexa.GetTileCenter (indexTile);
		...
		void Update () {
			...
			hexa.SetTileTextureRotation(MATHMAGIC_INDEX, Mathf.Deg2Rad * angle);
		}
		...
		void AssembleHexagram(int tileIndex) {
			...
			// Spawn 50 objects
				...
				GameObject obj = Instantiate(assemble);
		...
		void HexagonToken01(int tileIndex) {
			...
			GameObject obj = Instantiate(hexagonItemGameObject);
		...
		void HexagonToken02(int tileIndex) {
			...
			GameObject obj = Instantiate(hexagonExperienceGameObject);
